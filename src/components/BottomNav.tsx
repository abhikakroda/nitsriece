...
const BottomNav: React.FC = () => {
  ...
  return (
    <>
      {/* Mobile floating bar */}
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 lg:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex items-center gap-0.5 liquid-glass-float rounded-full px-1.5 py-1.5">
          {tabs.map(tab => {
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 rounded-full vibrant-gradient"
                    transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                  />
                )}
                <div className={`relative z-10 flex items-center gap-1.5 px-3.5 py-1.5 ${isActive ? '' : ''}`}>
                  <span className={`material-symbols-outlined text-[18px] transition-all duration-200 ${
                    isActive ? 'text-white fill-1' : 'text-muted-foreground/60'
                  }`}>{tab.icon}</span>
                  {isActive && (
                    <motion.span
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 rounded-xl vibrant-gradient"
