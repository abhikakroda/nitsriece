      ...
      <motion.div className="max-w-3xl mx-auto w-full relative z-10" variants={stagger} initial="hidden" animate="visible">
        ...
        {(currentClass || nextClass) && (
          <motion.div variants={fadeUp} className="px-5 md:px-6 pt-4 pb-2">
            {currentClass ? (
              <div className="rounded-2xl vibrant-gradient p-4 text-white shadow-lg">
                <div className="flex items-center gap-2 mb-2">
                  <span className="size-2 rounded-full bg-white animate-pulse" />
                  <span className="text-[10px] font-bold text-white/80 uppercase tracking-widest">Now</span>
                </div>
                <h3 className="font-display font-bold text-[18px] md:text-[20px] text-white leading-tight">{currentClass.subject}</h3>
                <div className="flex items-center gap-3 mt-1.5">
                  <span className="text-[11px] text-white/80">{currentClass.time}</span>
                  <span className="text-[11px] text-white/80">{currentClass.room}</span>
                  <span className="text-[11px] text-white/80">{currentClass.faculty || 'TBA'}</span>
                </div>
              </div>
            ) : nextClass ? (
