...
import { useMinimalistMode } from '@/hooks/use-minimalist';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button
          onClick={handleSave}
          className="flex-1 py-2.5 rounded-2xl vibrant-gradient text-[11px] font-bold active:scale-[0.97] transition-transform"
        >
          Save changes
        </button>
      </div>
...
const Profile: React.FC = () => {
  ...
  const { isMinimalist, toggleMinimalistMode } = useMinimalistMode();
      ...
      <motion.div className="max-w-3xl mx-auto w-full px-5 md:px-6 space-y-5 pt-6 pb-6" variants={stagger} initial="hidden" animate="visible">
        ...
        <motion.section variants={fadeUp}>
          ...
          <div className="liquid-glass rounded-2xl divide-y divide-white/15 dark:divide-white/5">
            ...
            <button className="flex items-center w-full gap-3 px-4 py-3.5 justify-between" onClick={toggleMinimalistMode}>
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-muted-foreground text-[20px]">format_color_fill</span>
                <div className="text-left">
                  <p className="text-foreground text-[13px] font-medium">Minimalist Mode</p>
                  <p className="text-muted-foreground text-[10px]">{isMinimalist ? 'Pure B&W active' : 'Default colors active'}</p>
                </div>
              </div>
              <div className={`w-[38px] h-[22px] rounded-full p-0.5 transition-colors ${isMinimalist ? 'bg-foreground' : 'bg-white/30 dark:bg-white/10'}`}>
                <motion.div className="h-[18px] w-[18px] rounded-full bg-background shadow-sm" animate={{ x: isMinimalist ? 16 : 0 }} transition={{ type: 'spring', stiffness: 500, damping: 35 }} />
              </div>
            </button>
