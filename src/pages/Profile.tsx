  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-20 lg:pb-6">
      ...
      <div className="flex items-center px-4 md:px-6 py-3 border-b border-border">
        <h2 className="text-foreground text-[17px] md:text-[19px] font-bold tracking-tight flex-1 text-center lg:text-left">Settings</h2>
      </div>

      <div className="max-w-3xl mx-auto w-full px-4 md:px-6 space-y-5 pt-5 pb-6">
        {/* Attendance Overview */}
        {statEntries.length > 0 && (
          <section>
            <h3 className="text-muted-foreground text-[10px] md:text-[11px] font-bold uppercase tracking-widest px-1 mb-2">Attendance Overview</h3>
              ...
              {statEntries.map(([subject, stats]) => (
                <div key={subject} className="flex items-center gap-3 px-4 py-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-foreground text-[13px] md:text-[14px] font-medium truncate">{subject}</p>
                    <p className="text-muted-foreground text-[10px] md:text-[11px]">{stats.present}P / {stats.absent}A — {stats.total} classes</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-16 md:w-24 h-1.5 bg-secondary rounded-full overflow-hidden">
                      <div
        ...
        <div className="md:grid md:grid-cols-2 md:gap-5 space-y-5 md:space-y-0">
          {/* Notifications */}
          <section>
            <h3 className="text-muted-foreground text-[10px] md:text-[11px] font-bold uppercase tracking-widest px-1 mb-2">Notifications</h3>
            <div className="bg-card rounded-2xl overflow-hidden divide-y divide-border border border-border">
              <div className="flex items-center gap-3 px-4 py-3.5 justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-[10px] md:text-[11px]">Get notified 20min, 5min before class</p>
                  </div>
                </div>
          ...
          {/* Appearance */}
          <section>
            <h3 className="text-muted-foreground text-[10px] md:text-[11px] font-bold uppercase tracking-widest px-1 mb-2">Appearance</h3>
            <div className="bg-card rounded-2xl overflow-hidden divide-y divide-border border border-border">
