...
@Composable
fun WeeklyScheduleEditorScreen(appViewModel: AppViewModel, onBack: () -> Unit) {
    ...
    fun copyToNextDay() {
        if (selectedDay != AppDay.Fri) {
            val nextDay = AppDay.entries[selectedDay.ordinal + 1]
            val currentDaySlots = working[selectedDay] ?: emptyList()
            working = working.toMutableMap().apply { put(nextDay, currentDaySlots) }
            selectedDay = nextDay
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            LaunchedEffect(selectedDay) {
                ...
                val targetPx = with(density) { (index * 64).dp.toPx() } // 56dp width + 8dp spacing
            ...
            Row(
                ...
                AssistChip(
                    ...
                    label = { Text("Copy to Next Day") },
                    enabled = selectedDay != AppDay.Fri
                )
            }
                ...
                if (slots.isEmpty()) {
                    item {
                        Box(
                            ...
                            Text(
                                "No classes for ${selectedDay.fullName}",
...
@Composable
private fun ConflictWarningCardWeekly(
    title: String,
...
@Composable
private fun ClassSlotCardWeekly(slot: ClassSlot, onEdit: () -> Unit, onDelete: () -> Unit) {
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                if (slot.faculty != null) {
                    Text("by ${slot.faculty}", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.primary)
                }
            }
...
@Composable
private fun DayTabWeekly(day: AppDay, isSelected: Boolean, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .height(56.dp)
            .width(56.dp)
